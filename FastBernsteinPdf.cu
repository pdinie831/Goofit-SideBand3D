...
// };
 ...
 __device__ fptype device_coeffbinomial_fast(fptype enne, fptype kappa){
 ...
 __device__ fptype  device_bernsteinkn_func(fptype x, fptype enne, fptype kappa){
 
   return device_coeffbinomial_fast(enne,kappa)*pow(x,kappa)*pow(1.0-x,enne-kappa);
...
//         }	 
        ...
        ifactni =  device_coeffbinomial_fast(enne,i);
// i!/(k!(i-k)!)
//         ifactik=1;
//         for(float l = 1; l <=k; ++l) {
//           ifactik *= (i+1-l)/l; 
//         }	
        ...
        ifactik =  device_coeffbinomial_fast(i,kappa);
//
...
//        }
       ...
       if ( enne>9) {
        printf("device_MultiBernstein error: Max(Numbers of degree) > 9 = %d\n",enne);
        return 0.0;
       }
...
//       for( int i = 1; i <= 9 ; ++i){
       for( int i = 1; i <= enne ; ++i){
        if(i<=maxDegree1) sx[i]= sx[i-1]*(1.-x);
        if(i<=maxDegree2) sy[i]= sy[i-1]*(1.-y);
        if(i<=maxDegree3) sz[i]= sz[i-1]*(1.-z);
       }
...
//       int jj = 0;
       ...
       for(int i = 0; i <= maxDegree1 ; ++i) {
         bernknvalx =  device_coeffbinomial_fast(maxDegree1,i)*tx*sx[maxDegree1-i];
//       jj = 0;
         fptype ty = 1.;
         for(int j = 0; j <= maxDegree2 ; ++j) {
 	  bernknvaly =  device_coeffbinomial_fast(maxDegree2,j)*ty*sy[maxDegree2-j];
...
//     	   fptype bernknvalz =  device_coeffbinomial_fast(maxDegree3,k)*pow(z,k)*pow(1.0-z,maxDegree3-k);
...
//     numParameters = (maxDegree1+1)*(maxDegree2+1)*(maxDegree3+1);
     ...
     cudaDeviceGetLimit(&limit, cudaLimitStackSize);
     printf("cudaLimitStackSize: %u\n", (unsigned)limit);
     printf("GooFit::Multivariate FastBernstein, num obs:%d\n",obses.size());
